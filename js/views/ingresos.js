...
// Refresca desde el servidor (llamado por app.js en cada ciclo de sync, ver
// EV.DATA_CHANGED-equivalente) — no bloquea si la pestaña no está abierta.
export async function refreshIngresos() {
  await _fetchAll();
  if (_root) _render();
}
...
async function _fetchAll() {
  try {
    ...
    console.error('[ingresos] carga falló', e);
    if (_cache === null) { _cache = []; toast.err('No se pudo cargar el historial de ingresos'); }
  } finally {
    _loading = null;
  }
}
...
function _render() {
  ...
  const statsHTML = `
    <div class="ing-stats-row">
      <div class="ing-stat"><div class="ing-stat-n">${totalUnidades.toLocaleString('es-VE')}</div><div class="ing-stat-l">unidades recibidas</div></div>
  ...
  if (!dates.length) {
    ...
    <div class="ing-diary-wrap">
      ...
      <div class="ing-diary-empty">
        ...
        <div style="font-size:15px;font-weight:600;color:var(--t2);margin-bottom:6px">Sin ingresos registrados</div>
        <div style="font-size:13px;color:var(--t4)">Lo que se registre en Ingreso Rápido aparecerá aquí agrupado por día.</div>
      </div>
    </div>`;
    return;
  }
  ...
  wrap.innerHTML = `
    ...
    <div class="ing-diary-header">
      <div class="ing-diary-header-info">
        <div class="ing-diary-header-title">Historial de recepciones</div>
    ...
    <div class="ing-diary-grid" id="ing-diary-grid"></div>`;
...
function _showDetail(date, records, isToday, isYest) {
  ...
  let ov = document.getElementById('ing-day-modal');
  if (!ov) {
  ...
  <div class="idm-box">
    <div class="idm-hero">
      ...
      <div class="idm-hero-actions">
        ...
        <button class="idm-close" data-close="ing-day-modal">
    ...
    <div class="idm-body">
      <div class="idm-section">
        <div class="idm-section-title">Lo más recibido</div>
        <div class="idm-bars">
          ${items.map((it, i) => {
            const w = Math.round(it.total / max * 100);
            const col = catColor(it.categoria);
...
function _exportDay(date, records) {
  ...
  ws['!cols'] = [{ wch:12 },{ wch:8 },{ wch:40 },{ wch:16 },{ wch:10 }];
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, 'Ingresos');
  XLSX.writeFile(wb, `ingresos-${date}.xlsx`);
