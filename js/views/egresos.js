...
// catálogo compartido puede haberlo entregado más de un grupo — hace falta
  ...
  try {
    ...
    for (;;) {
      const url = `${SUPABASE_URL}/rest/v1/movement_items`
        + `?select=id,qnty,movements!inner(id,occurred_at,note,direction,grupo_id,comandas(notas)),products(name,category_id,client_id)`
          ...
          ${timeline.map(r => `
            <div class="idm-tl-row">
              ...
              <div class="idm-tl-info">
                ...
                <div class="idm-tl-cat">Destino: ${r.destino ? escHtml(r.destino) : '—'}${showGrupo && r.grupoId != null ? ` · ${escHtml(_grupoLabel(r.grupoId))}` : ''}</div>
              </div>
