...
-- ============================================================
-- Egreso Rápido exigía una fila en `ubicaciones` marcada
-- es_default_egreso=true (destino resuelto del lado del servidor) y
-- fallaba por completo sin ella: "No hay una ubicación marcada como
-- destino por defecto de Egreso Rápido...". Este sistema todavía no
-- necesita registrar un destino/origen físico para el egreso — se
-- desacopla esa dependencia. Por ahora, una comanda de Egreso Rápido solo
-- registra: solicitante, quién autorizó (el creador), ítems con su
-- cantidad, id y fecha/hora de emisión. `comandas.ubicacion_id` y
-- `movements.destination` ya eran columnas nullable — no hace falta
-- alterar ninguna tabla, solo dejar de exigir/escribir ese valor acá.
--
...
begin
  ...
  -- Sin ubicación por ahora (ver comentario de cabecera) — ubicacion_id
  -- queda NULL (columna ya nullable).
